...
print_help() {
    ...
    echo ""
    echo "Options:"
    echo "  --device   CU118|CU121|CU124|CU126|CU128|ROCM|MPS|CPU    Specify the Device (REQUIRED)"
...
while [[ $# -gt 0 ]]; do
    ...
    --device)
        ...
        *)
            ...
            echo -e "${ERROR}Choose From: [CU118, CU121, CU124, CU126, CU128, ROCM, MPS, CPU]"
            exit 1
            ;;
        esac
        shift 2
...
if [ "$USE_CUDA" = true ] && [ "$WORKFLOW" = false ]; then
    if [ "$CUDA" = 128 ]; then
        ...
        echo -e "${INFO}Installing PyTorch For CUDA 12.4..."
        run_pip_quiet torch torchaudio --index-url "https://download.pytorch.org/whl/cu124"
    elif [ "$CUDA" = 121 ]; then
        echo -e "${INFO}Installing PyTorch For CUDA 12.1..."
        run_pip_quiet torch torchaudio --index-url "https://download.pytorch.org/whl/cu121"
    elif [ "$CUDA" = 118 ]; then
        echo -e "${INFO}Installing PyTorch For CUDA 11.8..."
        run_pip_quiet torch torchaudio --index-url "https://download.pytorch.org/whl/cu118"
    fi
